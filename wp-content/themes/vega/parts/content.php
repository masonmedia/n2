...
<div id="post-<?php the_ID(); ?>" <?php post_class('entry clearfix ' . $post_class); ?>>
    ...
    <!-- Post Title -->
    <h3 class="entry-title block-title block-title-left"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php esc_html_e('Post ID: ', 'vega'); echo $id; // PHPCS: XSS ok. ?></a></h3>
    <?php } else { ?>
    <h3 class="entry-title"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php esc_html_e('Post ID: ', 'vega'); echo $id; // PHPCS: XSS ok. ?></a></h3>
    <?php } ?>
    <!-- /Post Title -->
    ...
    <div class="entry-buttons <?php if($vega_wp_blog_feed_display == 'Small Image Left, Excerpt Right') { ?> entry-buttons-right <?php } ?>">
        <?php $readmore = vega_wp_get_option('vega_wp_blog_feed_readmore_text'); ?>
        <?php if($readmore != '') { ?><a href="<?php the_permalink(); ?>" class="btn btn-primary-custom btn-readmore"><?php echo esc_html( $readmore ); ?></a><?php } ?>
